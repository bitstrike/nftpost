#!/usr/bin/env python3
"""
nftpost -- iptables-paradigm nftables rule generator
...
class NftpostApp:
    ...
    def _do_generate(self):
        if not self.config_name:
            msg_dialog(self.stdscr, "Generate", ["No config loaded. Use s:save first."])
            return
        ...
        msg_dialog(self.stdscr, "Generate",
                   [f"Script written to:",
                    f"  {path}",
                    "",
                    "Apply with:  nft -f " + path,
                    "Or:          sudo sh " + path])
        self.info(f"Generated: {path}")
